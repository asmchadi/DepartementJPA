package com.department.entities;

import javax.persistence.Entity;
import javax.persistence.Table;

import com.department.utils.CommonWithFile;
import com.department.utils._TableNames;

@Entity
@Table(name = _TableNames.Type_Attestation)
public class TypeAttestation extends CommonWithFile{

	public TypeAttestation() {
		super();
		// TODO Auto-generated constructor stub
	}

	public TypeAttestation(Long id, String intitule, byte[] file_data,
			String file_name, String file_contentType) {
		super(id, intitule, file_data, file_name, file_contentType);
		// TODO Auto-generated constructor stub
	}

	public TypeAttestation(Long id, String intitule) {
		super(id, intitule);
		// TODO Auto-generated constructor stub
	}
}
